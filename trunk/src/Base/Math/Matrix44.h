...
public:
	...
	inline Matrix44<T>& SetFromQuatPos(const Quaternion<T>& quat, const Vector3<T>& vPos);
	inline Matrix44<T>& SetFromOrtho(float Width, float Height, float ZNear, float ZFar);
	...
	inline Matrix44<T>& SetRotByQuat      (const Quaternion<T>& quat);
