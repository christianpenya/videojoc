...
#define Enum_String_Id(EnumeratorName, IdEnum)                                  \
    RegisterEnumerator( EnumeratorName, IdEnum );
...
public:
	...
	static const std::string &ToStr(const EnumType e);

	// Converts from a string to an enumerator.
	// Returns true if the conversion is successful; false otherwise.
	static const bool ToEnum(EnumType &e, const std::string &str);
};
...
template <class D, class E>
const std::string &EnumStringBase<D, E>::ToStr(const E e)
{
	for (;;) // Code block
	{
...
template <class D, class E>
const bool EnumStringBase<D, E>::ToEnum(E &e, const std::string &str)
{
