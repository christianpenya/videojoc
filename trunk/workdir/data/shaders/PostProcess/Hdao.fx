...
float4 PSHDAO(PS_INPUT IN) : SV_Target
{
	...
	float l_CameraProjectionFarPlane = m_CameraInfo.y;
	float l_CameraProjectionNearPlane = m_CameraInfo.x;
	float l_Q= l_CameraProjectionFarPlane /( l_CameraProjectionFarPlane - l_CameraProjectionNearPlane);
	float l_QTimesZNear=l_Q*l_CameraProjectionNearPlane;
	...
	if( fDot > 0.5f )
	{
		...
		{
			f2MirrorScreenCoord = ( ( f2KernelScale * m_HDAORingPattern[iGather] ) +
			...
			if( m_HDAOUseNormals )
			{
				...
				f4SampledNormalZ[1] = GatherSamples( T1Texture, S1Sampler, f2MirrorTexCoord_10_1);
				// Scale normals
